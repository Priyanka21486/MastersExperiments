...
def extract_features_from_layer(audio_path, model, layer_index):
    ...
    inputs = feature_extractor(audio, return_tensors="pt", padding="longest",sampling_rate=16000)
...
def evaluate_layer(layer_index):
    """
    Extract features from both folders for a given layer index, train an SVM classifier,
    and print evaluation metrics.
    """
    folder_class_1 = "/home/spl_cair/Desktop/priyanka/icassp_exp/IED_PR"
    folder_class_2 = "/home/spl_cair/Desktop/priyanka/icassp_exp/TISA_PR"
    ...
    print(f"Layer {layer_index} Metrics:")
    print(f"f1: {f1:.4f}")
    print(f"auc: {auc:.4f}")
    print(f"conf: \n{conf}")
...
def main():
    for layer_index in range(1, 25):  # Loop from layer_index 1 to 24
        evaluate_layer(layer_index)
